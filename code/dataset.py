...
def custom_crop_img(img, vertices, labels, length):
    ...
    while flag and cnt < 1000:
        cnt += 1
        if rand == 1 :
            start_w = int(1 * remain_w)
            start_h = int(0 * remain_h)
        else :
            start_w = int(np.random.rand() * remain_w)
            start_h = int(np.random.rand() * remain_h)
        flag = is_cross_text([start_w, start_h], length, new_vertices[labels==1,:])
...
def split_full_data(seed, test_size, data_dir) :
    train_image_dir = data_dir + '/img/train'
...
class CustomSceneTextDataset(Dataset):
    def __init__(self, root_dir, image_files,
                 split='train',
                 image_size=2048,
                 crop_size=1024,
                 ignore_tags=[],
        ...
        self.image_fnames = sorted(image_files)
        self.image_dir = osp.join(root_dir, 'img', split)
    ...
    def __getitem__(self, idx):
        ...
        if self.normalize:
            funcs.append(A.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)))
        funcs.append(A.Blur(blur_limit=(11, 11), p=0.5))
            
        # Add augmentations 
        #funcs.append(A.Crop(x_min=512, y_min=0, x_max=1024, y_max=512, p=1))
        #funcs.append(A.Rotate(limit=(-30, 90), p=1)) ###
...
class PickleDataset(Dataset):
    def __init__(self, datadir, to_tensor=True):
        ...
        self.datalist = [i for i in range(240)]
        self.datalist.sort()
        self.to_tensor = to_tensor
        self.size_list = list(range(1280, 2304 + 1, 256))

    def __getitem__(self, idx):
        i = random.randint(0, 4)
        with open(file='/opt/ml/input/data/multi_scaled' + f"/{idx}_{self.size_list[i]}.pkl", mode="rb") as f:
            tup = pickle.load(f)
